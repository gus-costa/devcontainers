...
IMAGE="$1"
MANIFEST_FILE="images/${IMAGE}/manifest.json"

# Make manifest.json optional for GHCR-based publishing (specs/publishing.md)
if [[ ! -f "$MANIFEST_FILE" ]]; then
    echo "INFO: Manifest file not found: $MANIFEST_FILE"
    echo "Skipping tag validation (manifest not required for GHCR publishing)"
    exit 0
fi
